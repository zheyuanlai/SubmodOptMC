...
def batch_greedy(f, U, m, l, q):
    """
    Batch Greedy Algorithm.

    Parameters:
    f (function): Monotonically non-decreasing set function.
    U (set): Ground set of elements.
    m (int): Total cardinality constraint.
    l (int): Number of steps.
    q (list of int): Cardinality constraints for each step such that sum(q) = m.

    Returns:
    set: Selected subset of elements.
    """
    assert len(q) == l, "Length of q must be equal to l"
    assert sum(q) == m, "Sum of q must be equal to m"

    S = set()
    for i in range(l):
        gains = {e: f(S | {e}) - f(S) for e in U if e not in S}
        Q = set(sorted(gains, key=gains.get, reverse=True)[:q[i]])
        S.update(Q)
    return S
...
if __name__=="__main__":
    ...
    def f(S):
        _, piWS, PWS = keep_S_in_mat(P, state_space, pi, W.union(S))
        _, piW, PW = keep_S_in_mat(P, state_space, pi, W)
        _, piS, PS = keep_S_in_mat(P, state_space, pi, S)        
        return KL_divergence_gpu(piWS, PWS, compute_outer_product_gpu(PW, PS))
    ...
    for m in range(1, len(U) + 1):
        ...
        bg_subset = batch_greedy(f, U, m, l, q)
        print(f"m={m}; Greedy subset chosen: {greedy_subset}; Value: {f(greedy_subset)}")
        print(f"m={m}; Batch greedy subset chosen: {bg_subset}; Value: {f(bg_subset)}")
